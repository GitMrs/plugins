    ...
    <div class="player">
      <div class="player__top">
        <div class="player-cover">
          <transition-group :name="transitionNane">
            <div
        ...
        <div class="player-controls">
          <div class="player-controls__item -favorite" :class="{active: currentTrack.favorited}" @click="favorited">
            <svg class="icon">
              <use xlink:href="#icon-heart-o" />
            </svg>
          </div>
          <a target="_blank" :href="currentTrack.url" class="player-controls__item">
            <svg class="icon">
      ...
      <div class="progress" ref="progress">
        <div class="progress__top">
          <div class="album-info" v-if="currentTrack">
            <div class="album-info__name">{{this.currentTrack.artist}}</div>
            <div class="album-info__track">{{this.currentTrack.name}}</div>
          </div>
          <div class="progress__duration">{{duration}}</div>
        </div>
        <div class="progress__bar" @click="clickProgress">
          <div :style="{width:barWidth}" class="progress__current"></div>
        </div>
...
<script>
/**
 * 原链接https://codepen.io/JavaScriptJunkie/pen/qBWrRyg?editors=1010;
 * 感谢大佬的代码分享
 */
export default {
  name: "music",
  ...
  methods: {
    ...
    generateTime() {
      ...
      durmin = this.addZreo(durmin);
      dursec = this.addZreo(dursec);
      curmin = this.addZreo(curmin);
      cursec = this.addZreo(cursec);
  ...
  &-cover {
    ...
    &__item {
      ...
      &::after {
        ...
        width: 100%;
        height: 100%;
        box-shadow: 0 10px 40px 0px rgba(76, 70, 124, 0.5);
        display: block;
        z-index: 2;
        position: absolute;
        border-radius: 15px;
...
.album-info {
  ...
  padding-right: 60px;
  user-select: none;
  @media screen and (max-width: 576px), (min-height: 500px) {
    padding-right: 30px;
  }
  &__name {
    font-size: 20px;
